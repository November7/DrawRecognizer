(function () {
    ...
    function resolveInputShape(inputMeta, currentModelPath) {
        const dims = Array.isArray(inputMeta?.dimensions) ? inputMeta.dimensions : [];
        const readDim = (index, fallback) => readDimensionValue(dims[index], fallback);

        if (dims.length === 2) return [readDim(0, 1), readDim(1, 784)];
        if (dims.length === 3) return [readDim(0, 1), readDim(1, 28), readDim(2, 28)];
        if (dims.length === 4) return [readDim(0, 1), readDim(1, 28), readDim(2, 28), readDim(3, 1)];

        const loweredModelPath = String(currentModelPath || '').toLowerCase();
        const isTeachableMachineModel = loweredModelPath.includes('tm') || loweredModelPath.includes('teachable');
        return isTeachableMachineModel ? [1, 224, 224, 3] : [1, 28, 28, 1];
    }
    ...
    async function loadModelMetadata({ getModelAssetUrl, modelPath, nClasses }) {
        ...
        if (!Number.isInteger(nClasses) || nClasses <= 0) return [];
